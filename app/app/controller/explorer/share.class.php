...
class explorerShare extends Controller{
	...
	public function pathList(){
		$thePath = $this->in['path'];
		$pathParse = KodIO::parse($this->in['path']);
		if($pathParse['type'] == KodIO::KOD_SEARCH){
			$paramSearch = Action('explorer.listSearch')->parseSearch($pathParse['param']);
			$thePath = $paramSearch['parentPath'];
		}

		$this->parsePath($thePath); //校验path;
		$data = Action('explorer.list')->path($this->in['path']);
		show_json($data);
	}
